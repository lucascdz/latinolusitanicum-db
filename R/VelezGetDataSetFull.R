# arguments
# dictDataDir = path to the directory with TSV files exprtoed from Numbers file.
...
GetDataSet <- function(dictDataDir){
   ...
   ##### CONSIDER TO INSERT A 'rdfs.label' FOR FORMS TOO (MAYBE WHEN IT BECOMES FULL FORMS)
   ...
   LexicalSensesAllDF$rdfs.label <- unlist(lapply(seq_along(LexicalSensesAllDF$ontolex.LexicalSense), function(i) paste(LexicalSensesAllDF$skos.note[i], LexicalSensesAllDF$skos.definition.la[i], LexicalSensesAllDF$skos.definition.pt[i], LexicalSensesAllDF$lexinfo.note[i])))
   ...
   UsageExamplesDF <- UsageExamplesDF[,c(1,8,2:7)]

   DictDataDFsList <- list(EntriesDF=EntriesDF,ComponentsDF=ComponentsDF,LexicalEntriesDF=LexicalEntriesDF,FormsDF=FormsDF,LexicalSensesDF=LexicalSensesAllDF,LexicalConceptsDF=LexicalConceptsDF,UsageExamplesDF=UsageExamplesDF)
